...
if (!userData) {
  if (sessionStorage.getItem("poslovalnica") !== null) {
    imePoslovalnice = JSON.parse(
      sessionStorage.getItem("poslovalnica")
    ).poslovalnica;
  }
...
if (
  (window.location.pathname === "/predogledGosti" ||
    window.location.pathname === "/predogledGosti.html") &&
  imePoslovalnice !== "" &&
  !userData
) {
  const userDataDisplay = document.getElementById("userDataDisplay");
...
function logout() {
  // let canceled = false;
  if (
    window.location.pathname === "/urediTrenitenPlan" ||
    window.location.pathname === "/urediTrenitenPlan.html"
  ) {
    if (!dataSaved) {
      let answer = window.confirm(
        "Ali ste prepričani, da želite zapustiti stran? Pri tem lahko izgubite neshranjene podatke, ki ste jih vnesli."
      );
      if (answer) {
        sessionStorage.clear();
        window.location.href = "index.html";
        document
          .getElementById("logoutBlock")
...
function loginGuest() {
  ...
  const loginPoslovalnicaField = document.getElementById(
    "loginPoslovalnicaField"
  );
  const loginGuestPasswordField = document.getElementById(
    "loginGuestPasswordField"
  );
  ...
  xhr.onload = function (event) {
    ...
    // drugače shrani uporabniške podatke in osveži stran
      ...
      sessionStorage.setItem(
        "poslovalnica",
        JSON.stringify(serverRes.poslovalnica)
      );
      // window.location.href = "index.html";
      window.location.reload();
    }
